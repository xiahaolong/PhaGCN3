...
if torch.cuda.is_available():
    ...
    feature = torch.sparse_coo_tensor(i.t(), v, features[2], dtype=torch.float).to(device)

    i = torch.from_numpy(supports[0]).long().to(device)
    v = torch.from_numpy(supports[1]).to(device)
    support = torch.sparse_coo_tensor(i.t(), v, supports[2], dtype=torch.float).to(device)
    ...
    feature = torch.sparse_coo_tensor(i.t(), v, features[2], dtype=torch.float).float()

    i = torch.from_numpy(supports[0]).long()
    v = torch.from_numpy(supports[1])
    support = torch.sparse_coo_tensor(i.t(), v, supports[2], dtype=torch.float).float()
...
net.eval()
out = net((feature, support))
out = F.softmax(out, dim=1)  # 对输出进行Softmax激活，获得每个类的概率（新）
if torch.cuda.is_available():
...
pred = np.argmax(out, axis = 1)
score = np.max(out, axis=1)  # 获取每个预测的最大概率值（新）
mode = "testing"
if mode == "validation":
...
with open(f"{args.outpath}/prediction.csv", 'w') as f_out:
    ...
    for key in test_to_id.keys():
        if labels[test_to_id[key]] == -1:
            ...
            _ = f_out.write(str(key) + "," + str(pred_to_label[pred[test_to_id[key]]]) + "_like"+ "," + str(score[test_to_id[key]]) + "\n")#（新）
        else:
